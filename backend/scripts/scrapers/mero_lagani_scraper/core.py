...
"""
mero_lagani_scraper.py

A simple scraper for fetching the latest news articles from merolagani.com.
Provides functionality to retrieve titles, links, article content and postdate.

Example Usage:
    from pathlib import Path
    from mero_lagani_scraper import MeroLaganiScraper
...
class MeroLaganiScraper:
    ...
    Attributes:
        base_url (str): The root URL of the Ratopati website.
    ...
    def __init__(self) -> None:
        ...
        """
        self.base_url: str = "https://merolagani.com/"
        self.latest_news_url: str = self.base_url
    ...
    def get_news_content(self, soup: Optional[BeautifulSoup]) -> str:
        ...
        content_div = soup.select(".media-content")
        if content_div:
            return '\n\n'.join([content.get_text(strip=True) for content in content_div])
    ...
    def get_post_date(self, soup: Optional[BeautifulSoup]) -> str:
        ...
        date_element = soup.select_one("#ctl00_ContentPlaceHolder1_newsDate")
    ...
    def get_latest_news(self, limit: Optional[int] = None) -> List[News]:
        ...
        Returns:
            List[News]: A list of News objects, each containing:
                ...
                - ai_summary: AI-generated summary points.
                - image_url: URL of the main image associated with the article.
        """
        soup = self._get_news_soup(self.latest_news_url)
        if not soup:
            return []

        news_items = soup.select("#ctl00_ContentPlaceHolder1_tblRecentNews a")
        articles: List[News] = []

        for i, anchor in enumerate(news_items, start=1):
            if not anchor:
                continue
            link = self.base_url + str(anchor.get('href'))
